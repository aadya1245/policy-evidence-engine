...
export function coverage(claims,jurisdictions,dimensions,sources=fixtures.sources){const linked=linkClaims(claims,sources);const cells=jurisdictions.flatMap(j=>dimensions.map(d=>({jurisdiction:j,dimension:d,covered:linked.some(c=>c.jurisdiction===j&&c.dimension===d&&c.support!=='unsupported')})));return {cells,percent:cells.length?Math.round(100*cells.filter(x=>x.covered).length/cells.length):0,gaps:cells.filter(x=>!x.covered)} }
